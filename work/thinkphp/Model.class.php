...
class Model
{
    ...
    protected $cacheTableNames = array();
    
    protected $isIncrCacheVersion = true;
    
    protected $ignoreQueryFields = array();
    
    protected $queryOptions = array();
    
    protected $enableCacheVer = true;
     ...
     * @return string
     */
    private function _get_version_key($table_name)
    {
        return 'cache_version_of_' . $table_name;
    }
     ...
     * @return bool|string
     */
    public function getCacheVer($table_name)
    {
        return Cache::redis()->get($this->_get_version_key($table_name));
    }
    
    public function makeCacheKey($extra = '')
    {
        $cacheVers = '';
        foreach ($this->cacheTableNames as $table) {
            $table_cache_ver = $this->getCacheVer($table);
            if (empty($table_cache_ver)) {
                $table_cache_ver = (string)microtime(true);
                $this->saveCacheVersion($table, $table_cache_ver);
            }
            $cacheVers .= $table_cache_ver;
        }
        foreach ($this->ignoreQueryFields as $field) {
            if (isset($this->queryOptions['where'][$field])) {
                unset($this->queryOptions['where'][$field]);
            }
        }
        $cache_key=  md5(json_encode($this->queryOptions) . $extra . $cacheVers);
        $this->_clearCacheVerData();
        return $cache_key;
    }
     ...
     * 使用完一次清空缓存版本号相关的数据
     */
    private function _clearCacheVerData()
    {
        $this->enableCacheVer = true;
        $this->cacheTableNames = array();
        $this->ignoreQueryFields = array();
        $this->queryOptions = array();
    }
    
    /**
     * 更新数据表的缓存版本号
     * @param string $table_name 数据库中的完整表名
     * @param string $ver
     * @return bool
     */
    public function saveCacheVersion($table_name = '', $ver = '')
    {
        if (!$this->isIncrCacheVersion) {
            $this->incrCacheVer();
            return false;
        }
        $table_name = empty($table_name) ? $this->getTableName() : $table_name;
        $ver = empty($ver) ? (string)microtime(TRUE) : $ver;
        return Cache::redis()->set($this->_get_version_key($table_name), $ver, 0);
    }
    ...
    public static function D()
    {
        ...
        if (isset(self::$singleton[$modelName])) {
            return self::$singleton[$modelName];
        }
        self::$singleton[$modelName] = new static;
        return self::$singleton[$modelName];
    }
     ...
     * @param bool $flag
     */
    public function incrCacheVer($flag = true)
    {
        $this->isIncrCacheVersion = $flag;
        return $this;
    }
    
    /**
     * 设置不影响缓存版本号的where条件中的字段
     * @param $field
     */
    public function setIgnoreCacheVerField($field)
    {
        $this->ignoreQueryFields[] = $field;
    }
    ...
    public function select($options = array())
    {
        ...
        if (isset($options['cache'])) {
            ...
            $data = $this->isRedisOn() ? json_decode(Cache::redis()->get($key), true) : S($key, '', $cache);
            if (!empty($data)) {
                return $data;
            }
        }
        ...
        if (isset($cache)) {
            $this->isRedisOn() ? json_decode(Cache::redis()->set($key, $resultSet), true) : S($key, $resultSet, $cache);
        }
        return $resultSet;
    }
    
    protected function _setCacheTableNames($options)
    {
        $this->queryOptions = $options;
        $regex = '/hm_[_a-zA-Z0-9]+/';
        if (empty($options)) {
            return;
        }
        //多表join查询
        if (isset($options['table'])) {
            $this->_regexAllTableNames($regex, $options['table']);
        }
        if (isset($options['join']) && !empty($options['join'])) {
            foreach ($options['join'] as $join) {
                $this->_regexAllTableNames($regex, $join);
            }
        }
        //todo union查询待处理...
    }
    
    protected function _regexAllTableNames($regex, $str)
    {
        preg_match_all($regex, $str, $matchs);
        foreach ($matchs[0] as $match) {
            if (!empty($match) && !in_array($match, $this->cacheTableNames))
                $this->cacheTableNames[] = $match;
        }
    }
     ...
     * @return array
     */
    protected function _parseOptions($options = array(), $mark = '')
    {
        ...
        if (!isset($options['table'])) {
            ...
            $this->useSubQuery || $fields = $this->getDbFields();
        }
        ...
        if ($this->isRedisOn() && !defined('ORM_FROM')) {
            $this->_setCacheTableNames($options);
            empty($mark) && $mark = "select";
            $options['cache']['key'] = $this->makeCacheKey($mark);
        }
    ...
    protected function isRedisOn()
    {
        return C('redis.on') && $this->enableCacheVer;
    }
     ...
     * @return $this
     */
    public function disableCacheVer()
    {
        $this->enableCacheVer = false;
        return $this;
    }
    ...
    public function find($options = array())
    {
        ...
        $options['limit'] = 1;
        // 分析表达式
        $options = $this->_parseOptions($options, 'find');
        ...
        $this->data = $data;
        if (isset($cache)) {
            $this->isRedisOn() ? Cache::redis()->set($key, $data) : S($key, $data, $cache);
        }
        return $this->data;
    }
    ...
    {
        $options['field'] = $field;
        $key_mark= ($sepa === true ? "getAll" : $sepa);
        $options = $this->_parseOptions($options, 'getField_' . $key_mark);
        // 判断查询缓存
        if (isset($options['cache'])) {
            $cache = $options['cache'];
            $key = is_string($cache['key']) ? $cache['key'] : md5($key_mark . serialize($options));
            $data = $this->isRedisOn() ? json_decode(Cache::redis()->get($key), true) : S($key, '', $cache);
            if (!empty($data)) {
                return $data;
            }
        ...
        if (strpos($field, ',') && false !== $sepa) { // 多字段
            ...
            if (!empty($result)) {
                if (true !== $sepa && 1 == $options['limit']) {
                    $data = reset($result[0]);
                    if (isset($cache)) {
                        $this->isRedisOn() ? Cache::redis()->set($key, is_array($data) ? $data : json_encode($data)) : S($key, $data, $cache);
                    }
                    return $data;
                }
                ...
                if (isset($cache)) {
                    $this->isRedisOn() ? Cache::redis()->set($key, $array) : S($key, $array, $cache);
                }
                return $array;
            }
        }
